...
module.exports = updateIndustryGroupData;
...
function updateIndustryGroupData(importedData) {
	return new Promise((resolve, reject) => {
		parseDWIndustryGroups(importedData)
			.then((response) =>{
				emitter.emit('buildIndustryGroup', response);
					bulkIndustryGroupsUpdate(response.array, 'industryGroups')
						.then(function (ids) {
							resolve(ids)
						})
...
function bulkIndustryGroupsUpdate(data) {
	return new Promise((resolve, reject) => {
		let promiseArray = [];
		for (let i = 0; i < data.length; i++) {
			promiseArray.push(database.industryGroups.update(data[i], {where: {sector: data[i].sector}}));
		}
